...
//router.use here is applying middleware to all routes in the playlists router (extracts user from token and check if they are logged in)
...
router.param("id", async (req, res, next, id) => {
  ...
  if (playlist.user_id !== req.user.id) {
    return res
      .status(403)
      .send("You don't have permission to access this playlist");
  }
